...
setwd("C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/Cluster_Markers/Alpaca_Human/Alpaca_Human_Comparison")
...
Alpaca_Human <- bind_rows(filelist, .id = "column_label")

DF <-Alpaca_Human %>% mutate(Alpaca = case_when(Cluster_A == 0 ~ "Cluster 0",
...
Heatmap <- DF %>%
  group_by(Alpaca, Human) %>%
  count(column_label) %>%
  dplyr::select(Alpaca, n)

Heatmap[is.na(Heatmap)] = 0

#Having the data as a matrix
Plot <- spread(Heatmap, Alpaca, n)
Plot[is.na(Plot)] = 0
...
heatmap(Plot, scale = "column", margins = c(9,9), xlab = "Human", ylab = "Alpaca", col= viridis(5))
legend(x="bottomright", legend=c("low", "high"), 
       fill=viridis(5))


#GGplot Heatmap
Heatmap %>% ggplot(aes(x = Alpaca, y = Human, fill = n))+
  geom_tile()+
  scale_fill_viridis()
...
pdf(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Llama_alpaca_both_virus/cluster_comparison_dim_30.pdf",
    width=11, height=6)
superheat(X = Plot,
          X.text = Plot,
          X.text.size = 6,
...
setwd("C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/Cluster_Markers/Alpaca_Human/Single_cluster_data")
temp = list.files(pattern="*.txt")
names(temp) <- temp
...
cluster0_alpaca <- Alpaca_Human %>%
  subset(Species == "Alpaca") %>%
  subset(Cluster == 0) %>%
  select(2,)
cluster1_alpaca <- Alpaca_Human %>%
  subset(Species == "Alpaca") %>%
  subset(Cluster == 1) %>%
  select(2,)
cluster2_alpaca <- Alpaca_Human %>%
  subset(Species == "Alpaca") %>%
  subset(Cluster == 2) %>%
  select(2,)
cluster3_alpaca <- Alpaca_Human %>%
  subset(Species == "Alpaca") %>%
  subset(Cluster == 3) %>%
  select(2,)
cluster4_alpaca <- Alpaca_Human %>%
  subset(Species == "Alpaca") %>%
  subset(Cluster == 4) %>%
  select(2,)
# cluster5_alpaca <- Alpaca_Human %>%
#   subset(Species == "Alpaca") %>%
#   subset(Cluster == 5) %>%
#   select(2,)
...
cluster0_human <- Alpaca_Human %>%
  subset(Species == "Human") %>%
  subset(Cluster == 0) %>%
  select(2,)
cluster1_human <- Alpaca_Human %>%
  subset(Species == "Human") %>%
  subset(Cluster == 1) %>%
  select(2,)
cluster2_human <- Alpaca_Human %>%
  subset(Species == "Human") %>%
  subset(Cluster == 2) %>%
  select(2,)
cluster3_human <- Alpaca_Human %>%
  subset(Species == "Human") %>%
  subset(Cluster == 3) %>%
  select(2,)
cluster4_human <- Alpaca_Human %>%
  subset(Species == "Human") %>%
  subset(Cluster == 4) %>%
  select(2,)
cluster5_human <- Alpaca_Human %>%
  subset(Species == "Human") %>%
  subset(Cluster == 5) %>%
  select(2,)
