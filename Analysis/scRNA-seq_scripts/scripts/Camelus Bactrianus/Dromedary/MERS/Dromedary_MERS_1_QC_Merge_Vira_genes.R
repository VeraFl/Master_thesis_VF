...
for (file in c("mock_filtered_feature_bc_matrix", "inf_filtered_feature_bc_matrix")){
  seurat_data <- Read10X(data.dir = paste0("data/C. Dromedarius/MERS_exon/", file))
...
inf_filtered_feature_bc_matrix@assays$RNA@counts[29363:29375,]

#take the right rows in the matrix
Infected_viral_counts <- as.matrix(inf_filtered_feature_bc_matrix@assays$RNA@counts[29363:29375,])
Mock_viral_counts <- as.matrix(mock_filtered_feature_bc_matrix@assays$RNA@counts[29363:29375,])
...
plot(sort(Infected_viral_percentage), xlim = c(0, 4500), xlab='cell', main='Viral.Percentage vs cell (ordered) - MERS infected')
...
inf_filtered_feature_bc_matrix@meta.data <- inf_filtered_feature_bc_matrix@meta.data %>% mutate(Status = case_when(Percent.Viral > 0 ~ "Infected",
                                                                       Percent.Viral <= 0 ~ "Bystander"))
