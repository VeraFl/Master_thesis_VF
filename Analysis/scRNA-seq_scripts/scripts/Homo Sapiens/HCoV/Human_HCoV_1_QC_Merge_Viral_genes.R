...
#Treatment colors(HCoV, Mock)
cols_treat = c("#84A59D", "#96ADC8")
show_col(cols_treat)
#Status colors
cols_stat = c("#B8544F","#D6B59F")
show_col(cols_stat)
#Celltype colors
...
###################################################################################
# #selecting the HCoV genes
#when there are more than one (all the HCoV genes seperately):

#calculating some info about the expression of the HCoV genes and gene content in general
...
#confirms the output summary, shows problems with gene and cell counts
plot(sort(genes_per_cell_inf), ylim = c(1, 10000), xlim = c(0, 5000), xlab='cell', log='y', main='genes per cell (ordered) - HCoV229E infected') #corrected
plot(sort(genes_per_cell_mock), ylim = c(1, 10000), xlim = c(0, 5000), xlab='cell', log='y', main='genes per cell (ordered) - HCoV229E mock')


###################################################################################################################
# Adding important info to the meta data column
#show the number of rows of the matrix to locate the HCoV genes
nrow(mock_filtered_feature_bc_matrix@assays$RNA@counts) #checking how many genes are in the sample
tail(counts_per_gene_inf, 15) #displaying the last 15 rows, where the HCoV genes are
tail(counts_per_gene_mock, 15) #displaying the last 15 rows, where the HCoV genes are

#show the specific location in the matrix for control
inf_filtered_feature_bc_matrix@assays$RNA@counts[19989:19997,]

#take the right rows in the matrix
Infected_HCoV_counts <- as.matrix(inf_filtered_feature_bc_matrix@assays$RNA@counts[19989:19997,])
Mock_HCoV_counts <- as.matrix(mock_filtered_feature_bc_matrix@assays$RNA@counts[19989:19997,])
#Count the HCoV counts
infected.total.HCoV.counts <- Matrix::colSums(Infected_HCoV_counts)
...
infected.percent.HCoV <- infected.total.HCoV.counts/infected.total.counts
...
inf_filtered_feature_bc_matrix <- AddMetaData(object = inf_filtered_feature_bc_matrix, metadata = infected.total.HCoV.counts, col.name = "Total.HCoV.Counts")
inf_filtered_feature_bc_matrix <- AddMetaData(object = inf_filtered_feature_bc_matrix, metadata = infected.percent.HCoV, col.name = "Percent.HCoV")
inf_filtered_feature_bc_matrix[["percent.mt"]] <- PercentageFeatureSet(inf_filtered_feature_bc_matrix, pattern = "^MT-")
inf_filtered_feature_bc_matrix <- AddMetaData(inf_filtered_feature_bc_matrix, "HCoV-229E", col.name = "Treat")
...
#in the HCoV infected Treatment group or non-infected in the Mock Group

#Extracting the HCoV-Percentage column for every cell and transpose it to plot it below
Infected_HCoV_percentage <- inf_filtered_feature_bc_matrix@meta.data %>% select(5) %>% t()
Mock_HCoV_percentage <- mock_filtered_feature_bc_matrix@meta.data %>% select(5) %>% t()

#plotting how many virus positive cells there are in each treatment
plot(sort(Infected_HCoV_percentage), xlim = c(0, 5000), xlab='cell', main='HCoV.Percentage vs cell (ordered) - HCoV infected')
plot(sort(Mock_HCoV_percentage), xlim = c(0, 5000), xlab='cell', main='HCoV.Percentage vs cell (ordered) - HCoV Mock')

#############################################################################################################################
#Calculation and visualization of infection threshold for the Human samples
#infected HCoV for HCoV
HCoV_infected_percentage <- inf_filtered_feature_bc_matrix@meta.data %>% select(5)

#calculate minimal number after 0 to set instead of 0, so they are included into plot
halfmin_HCoV <- min(HCoV_infected_percentage$Percent.HCoV[HCoV_infected_percentage$Percent.HCoV>0])
halfmin_HCoV

HCoV_infected_percentage$Percent.HCoV_log10 <- log10(HCoV_infected_percentage$Percent.HCoV+halfmin_HCoV)
range(HCoV_infected_percentage$Percent.HCoV_log10)
range(HCoV_infected_percentage$Percent.HCoV_l)

des.all <- density(HCoV_infected_percentage$Percent.HCoV_log10)
...
HCoV_infected_percentage$group = ifelse(HCoV_infected_percentage$Percent.HCoV_log10 <= min.all[4], 'Non-infected', ifelse(HCoV_infected_percentage$Percent.HCoV_log10, 'Infected'))
mock_percentage$group = ifelse(mock_percentage$Percent.HCoV_log10 <= min.all[4], 'Non-infected', ifelse(mock_percentage$Percent.HCoV_log10 > min.all[2], 'Infected'))

p1 <- HCoV_infected_percentage %>%
...
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/threshold_HCoV_infection.png",
...
10^(-3.234471)
#HCoV Infection threshold: 0.0005828127


#Threshold above 0.002 Percent HCoV is infected

inf_filtered_feature_bc_matrix@meta.data <- inf_filtered_feature_bc_matrix@meta.data %>% mutate(Status = case_when(Percent.HCoV > 0.0005828127 ~ "Infected",
                                                                                                                   Percent.HCoV <= 0.0005828127 ~ "Non-infected"))
...
# VlnPlot(inf_filtered_feature_bc_matrix, features = c("Percent.HCoV", "Total.HCoV.Counts"), ncol = 2)
# VlnPlot(inf_filtered_feature_bc_matrix, features = c("Percent.HCoV", "Total.HCoV.Counts"), ncol = 2)
# VlnPlot(inf_filtered_feature_bc_matrix, features = c("Percent.HCoV", "Total.HCoV.Counts"), y.max = 0.0005, ncol = 2)
# #mock
# VlnPlot(mock_filtered_feature_bc_matrix, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
# VlnPlot(mock_filtered_feature_bc_matrix, features = c("Percent.HCoV", "Total.HCoV.Counts"), ncol = 2)
# VlnPlot(mock_filtered_feature_bc_matrix, features = c("Percent.HCoV", "Total.HCoV.Counts"), ncol = 2)
# VlnPlot(mock_filtered_feature_bc_matrix, features = c("Percent.HCoV", "Total.HCoV.Counts"), y.max = 0.0005, ncol = 2)
# 
# 
...
# plot3 <- FeatureScatter(inf_filtered_feature_bc_matrix, feature1 = "Percent.HCoV", feature2 = "percent.mt")
# plot1 + plot2 + plot3
# #mock
...
#Plots for QC
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/cell_numbers_per_virus_before_filtering.png",
...
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/UMI_per_cell_density_log_before_filtering.png",
    width=600, height=500)
metadata %>% 
...
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/genes_per_cell_density_log_before_filtering.png",
    width=600, height=500)
metadata %>% 
...
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/complexity_log_before_filtering.png",
...
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/Mitochondrial_content_density_before_filtering.png",
    width=700, height=500)
metadata %>% 
...
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/correlation_gene_UMI_log_before_filtering.png",
...
HCoV.list <- c(mock_filtered_feature_bc_matrix, inf_filtered_feature_bc_matrix)
# normalize and identify variable features for each dataset independently
HCoV.list <- lapply(X = HCoV.list, FUN = function(x) {
  x <- NormalizeData(x)
  x <- CellCycleScoring(x, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)
  x <- SCTransform(x, method = "glmGamPoi", vars.to.regress = c("S.Score", "G2M.Score", "percent.mt"), verbose = FALSE)
})

# select features that are repeatedly variable across datasets for integration
features <- SelectIntegrationFeatures(object.list = HCoV.list, nfeatures = 3000)
HCoV.list <- PrepSCTIntegration(object.list = HCoV.list, anchor.features = features)
HCoV.anchors <- FindIntegrationAnchors(object.list = HCoV.list, anchor.features = features, normalization.method = "SCT")

# this command creates an 'integrated' data assay
HCoV.combined <- IntegrateData(anchorset = HCoV.anchors, normalization.method = "SCT")


###Perform an integrated analysis
# specify that we will perform downstream analysis on the corrected data note that the
# original unmodified data still resides in the 'RNA' assay
DefaultAssay(HCoV.combined) <- "integrated"

head(HCoV.combined@meta.data)
###################################################################################################################
...
SaveH5Seurat(HCoV.combined, "HCoV.combined", overwrite = TRUE)

#########################################################################################################
setwd("C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/seurat_objects/H.sapiens")
HCoV.combined <- LoadH5Seurat("HCoV.combined.h5seurat")
HCoV.combined


# Add number of genes per UMI for each cell to metadata
HCoV.combined$log10GenesPerUMI <- log10(HCoV.combined$nFeature_RNA) / log10(HCoV.combined$nCount_RNA)


metadata <- HCoV.combined@meta.data
# Rename columns
metadata <- metadata %>%
  dplyr::rename(seq_folder = orig.ident,
                nUMI = nCount_RNA,
...
#Plots for QC
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/cell_numbers_per_virus_after_filtering.png",
    width=600, height=500)
metadata_count %>% 
  ggplot(aes(x=Treat, y=n, fill=Status)) + 
  geom_col() +
...
#Number of UMI (transcripts) per cell
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/UMI_per_cell_density_log_after_filtering.png",
    width=600, height=500)
metadata %>% 
  ggplot(aes(color=Treat, x=nUMI, fill= Treat)) + 
  geom_density(alpha = 0.6) + 
...
#Number of Genes per cell
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/genes_per_cell_density_log_after_filtering.png",
    width=600, height=500)
metadata %>% 
  ggplot(aes(color=Treat, x=nGene, fill= Treat)) + 
  geom_density(alpha = 0.6) +
...
#Complexity
# Visualize the overall complexity of the gene expression by visualizing the genes detected per UMI (novelty score)
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/complexity_log_after_filtering.png",
    width=700, height=500)
metadata %>%
  ggplot(aes(x=log10GenesPerUMI, color = Treat, fill=Treat)) +
  geom_density(alpha = 0.6) +
...
# Visualize the distribution of mitochondrial gene expression detected per cell
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/Mitochondrial_content_density_after_filtering.png",
    width=700, height=500)
metadata %>% 
  ggplot(aes(color=Treat, x=percent.mt, fill=Treat)) + 
  geom_density(alpha = 0.6) + 
...
# Visualize the correlation between genes detected and number of UMIs and determine whether strong presence of cells with low numbers of genes/UMIs
png(file="C:/Users/veraf/OneDrive - Universitaet Bern/Single_Cell_CoV/Analysis/scRNA_seq/figures/Human_HCoV/correlation_gene_UMI_log_after_filtering.png",
    width=1000, height=500)
metadata %>% 
  ggplot(aes(x=nUMI, y=nGene, color = Treat)) + 
  geom_point() + 
