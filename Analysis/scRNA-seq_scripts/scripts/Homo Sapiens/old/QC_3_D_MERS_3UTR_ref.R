...
#   seurat_data <- Read10X(data.dir = paste0("data/H. sapiens/MERS_3UTR_exon/", file))
...
inf_seurat_data <- Read10X(data.dir = "data/H. sapiens/MERS_3UTR_exon/inf_filtered_feature_bc_matrix")
mock_seurat_data <- Read10X(data.dir = "data/H. sapiens/MERS_3UTR_exon/mock_filtered_feature_bc_matrix")
inf_seurat_obj <- CreateSeuratObject(counts = inf_seurat_data,min.features = 100,project = "inf_filtered_feature_bc_matrix")
mock_seurat_obj <- CreateSeuratObject(counts = mock_seurat_data,min.features = 100,project = "mock_filtered_feature_bc_matrix")


#calculating counts per gene and checking the viral genes if we have counts for them.
infected_counts_per_gene <- Matrix::rowSums(inf_seurat_data)
virus_genes_infected <- tail(infected_counts_per_gene, 11) %>% data.frame()
virus_genes_infected

mock_counts_per_gene <- Matrix::rowSums(mock_seurat_data)
virus_genes_mock <- tail(mock_counts_per_gene, 11) %>% data.frame()
virus_genes_mock
...
# Add column for Mitochondrial Content, viral content in percentage and treatment state to the meta data
...
#adding viral gene content in percentage
inf_filtered_feature_bc_matrix[["percent.v"]] <- PercentageFeatureSet(inf_filtered_feature_bc_matrix, pattern = "3UTR")
#add treatment state
inf_filtered_feature_bc_matrix <- AddMetaData(mock_filtered_feature_bc_matrix, "MOCK", col.name = "Treat")
...
#adding viral gene content in percentage
mock_filtered_feature_bc_matrix[["percent.v"]] <- PercentageFeatureSet(mock_filtered_feature_bc_matrix, pattern = "3UTR")
#add treatment state
mock_filtered_feature_bc_matrix <- AddMetaData(mock_filtered_feature_bc_matrix, "INFECTED", col.name = "Treat")
head(mock_filtered_feature_bc_matrix@meta.data)



VlnPlot(inf_filtered_feature_bc_matrix, features = "percent.v")
VlnPlot(mock_filtered_feature_bc_matrix, features = "percent.v")

RidgePlot(inf_filtered_feature_bc_matrix, features = "percent.v")
...
plot1 <- FeatureScatter(inf_filtered_feature_bc_matrix, feature1 = "percent.v", feature2 = "percent.mt")
...
# adding the viral genes
v.genes <- c("5UTR", "orf1ab", "S", "orf4a", "orf4b", "orf5", "E", "M", "N", "orf8b", "3UTR")
